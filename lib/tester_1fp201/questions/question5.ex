defmodule Tester1fp201.Questions.Question5 do
  ...
  def render_question(assigns) do
    ...
    <div>
      <p>
        Rozhodněte se mezi tím, zda a) vložit dnes <%= @deposit %> Kč do investice <em>Malina</em>
        s předpokládaným jednorázovým čistým příjmem ve výši <%= @yield %> Kč za <%= @years %> let (od zdanění se zde abstrahujte),
        nebo b) upřednostnit alternativní investici <em>Ostružina</em>
        s výnosovou (tj. zvažovanou) úrokovou mírou ve výši <%= @interest_rate %>%
        (zde zvažujte zdanění úroků ve výši <%= @tax %>%).
      </p>
    </div>
    """
  end
