  ...
  def render_solution(%{interest_rate: i}) do
    ...
    <div>
      ...
      <p>
        ...
        \]
        \[
        \begin{align*} <%= @eum_dc %> &= (1 + \frac{\text{NÚM}_{QC}}{4})^{4} - 1 \\ <%= @eum_dc1 %> &= (1 + \frac{\text{NÚM}_{QC}}{4})^{4} \\
        \sqrt[4]{<%= @eum_dc1 %>} &= 1 + \frac{\text{NÚM}_{QC}}{4} \\ <%= @eum_dc_sqrt1 %> &= 1 + \frac{\text{NÚM}_{QC}}{4} \\
        \text{NÚM}_{QC} &= <%= @eum_dc_sqrt %> \cdot 4 \\
        \text{NÚM}_{QC} &= <%= @num_qc %> \end{align*}
        \]
      </p>
    </div>
    """
