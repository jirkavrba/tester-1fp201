    ...
    <div>
      <p>
        Za kolik (tj. otázka na současnou hodnotu domu) jste vlastně prodali Váš bývalý dům,
         pokud první platba provedená dnes byla ve výši <strong><%= @first_payment %> Kč</strong> a dále budou následovat
         <strong><%= @years %></strong> platby po <strong><%= @additional_payments %> Kč</strong> na konci každého roku?
      </p>
      <p>
        Zvažujete <strong><%= @interest_rate %>% roční nominální úrokovou míru</strong> s <strong>ročním</strong> skládáním úroků. Od zdanění abstrahujete.
      </p>
    </div>
    """
  end
